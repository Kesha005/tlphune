...
class basecontrol extends Controller
{
    ...
    {
        $events = events::with('user', 'mark')->where('category_id', $id)->where('status', 1)->pluck('id','name','public_image','price','status')->map(function ($query) {
    ...
    public function filter($category_id, $mark_id)
    {
        ...
        $events = events::where('status', 1)->where('category_id', $category_id)->where('mark_id', $mark_id)->pluck('id','name','public_image','price','status')->map(function ($query) {
            return (array)($query->toArray() + ['is_new' => false]);
