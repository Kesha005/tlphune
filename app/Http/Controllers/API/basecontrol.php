...
class basecontrol extends Controller
{
    ...
    {
        $events = events::with('shop','etrap')->where('category_id', $id)->where('status', 1)->orderBy('created_at', 'DESC')->get(['id','name','public_image','user_id','is_new','price','place','created_at','updated_at','mark_id','category_id','vip'])->map(function ($item) {
            if($item->etrap)
            {
    ...
    public function filter($category_id, $mark_id)
    {
        ...
        $events = events::with('shop','etrap')->where('status', 1)->where('category_id', $category_id)->where('mark_id', $mark_id)->orderBy('created_at', 'DESC')->get(['id','name','public_image','user_id','is_new','price','place','created_at','updated_at','mark_id','category_id','vip'])->map(function ($item) {
            if($item->etrap)
            {
                $welayat=welayat::find($item->etrap->welayat_id); $place=$welayat->name;
            }
