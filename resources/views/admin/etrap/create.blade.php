...
<div class="modal fade" id="cmmodal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="{{route('admin.etrap.store')}}" enctype="multipart/form-data" id="markform" method="POST">
                @csrf
                ...
                <div class="modal-body">
                    ...
                    <label for="category">Welaýat</label>
                        <select name="welayat_id" class="form-control" id="category">
                            @foreach($welayats as $welayat)
                            <option value="{{$welayat->id}}" class="form-control">{{$welayat->name}}</option>
                            @endforeach
                        </select>
                    </div><br>
